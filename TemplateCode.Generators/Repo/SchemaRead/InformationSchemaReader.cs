...
namespace TemplateCodeGenerator.SchemaRead {
	public class InformationSchemaReader : SchemaReader {
		...
		public InformationSchemaReader(string connectionString, string providerName, ICrossGeneratorNameHandler nameHandler) : base(nameHandler) {
			ConnectionString = connectionString;
			ProviderName = providerName;
		}

		~InformationSchemaReader() {
		}
			...
			try {
				...
				using (var conn = _factory.CreateConnection()) {
					...
					foreach (var t in result.All) {
						...
						t.LowerCamelClassName = SpaceStripAndCamelCase(t.Name, true);
			...
			using (cmd) {
				...
				using (var rdr = cmd.ExecuteReader()) {
					while (rdr.Read()) {
						...
						tbl.LowerCamelClassName = this.SpaceStripAndCamelCase(tbl.Name, true);
		...
		List<Key> LoadOuterKeys(Table tbl) {
			using (var cmd = _factory.CreateCommand()) {
				...
				using (IDataReader rdr = cmd.ExecuteReader()) {
					Key currentKey = null;
					while (rdr.Read()) {
						var thisKeyName = rdr["FK"].ToString();
						var referencedTable = rdr["Referenced_tbl"].ToString();
						var referencingTable = rdr["Referencing_tbl"].ToString();

						if (currentKey == null) {
							currentKey = new Key() {
								Name = thisKeyName,
								ReferencedTableName = rdr["Referenced_tbl"].ToString(),
								ReferencingTableName = rdr["Referencing_tbl"].ToString()
							};
							result.Add(currentKey);
						}
						else if (!thisKeyName.Equals(currentKey.Name, StringComparison.CurrentCultureIgnoreCase)
							&& (!currentKey.ReferencedTableName.Equals(referencedTable, StringComparison.CurrentCultureIgnoreCase)
								|| !currentKey.ReferencingTableName.Equals(referencingTable, StringComparison.CurrentCultureIgnoreCase))) {
							var key = new Key() {
								Name = thisKeyName,
								ReferencedTableName = rdr["Referenced_tbl"].ToString(),
								ReferencingTableName = rdr["Referencing_tbl"].ToString()
						...
						currentKey.ReferencedTableColumnNames.Add(rdr["Referenced_col"].ToString());
						currentKey.ReferencingTableColumnNames.Add(rdr["Referencing_col"].ToString());
					}
				}
		...
		const string COLUMN_SQL = " SELECT DISTINCT " +
								...
								"	case when k.COLUMN_NAME IS NULL THEN 0 ELSE 1 end AS IsPk " +
		...
		const string OUTER_KEYS_SQL = "SELECT  " +
									...
									"ORDER BY " +
									"	OBJECT_SCHEMA_NAME(pt.constraint_object_id), " +
									"	OBJECT_NAME(pt.constraint_object_id), " +
									"	OBJECT_NAME(pt.referenced_object_id), " +
									"	pc.name, " +
									"	OBJECT_NAME(pt.parent_object_id),  " +
									"	rc.name;";
		...
		const string INNER_KEYS_SQL = "SELECT  " +
									"	[Schema] = OBJECT_SCHEMA_NAME(pt.constraint_object_id), " +
									"	FK = OBJECT_NAME(pt.constraint_object_id), " +
									"	Referenced_tbl = OBJECT_NAME(rc.object_id), " +
									...
									"	OBJECT_SCHEMA_NAME(pt.constraint_object_id), " +
									"	OBJECT_NAME(pt.constraint_object_id), " +
									"	OBJECT_NAME(rc.object_id), " +
									"	rc.name, " +
									"	OBJECT_NAME(pt.parent_object_id), " +
									"	pc.name; ";

		const string PK_SQL = "select " +
							"	c.TABLE_SCHEMA,  " +
							"	c.TABLE_NAME,  " +
							"	c.COLUMN_NAME " +
