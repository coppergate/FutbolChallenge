...
namespace TemplateCodeGenerator.SchemaRead {
	public class Table {
		...
		public void BuildSql() {
			...
			UpdateParameter = string.Join(", ", Columns.Where(k => !k.IsAutoIncrement && !k.Ignore && !k.IsComputed && !k.IsComputed).Select(c => $"[{c.Name}] = @{c.PropertyName}"));
			ValueParameter = string.Join(", ", Columns.Where(k => !k.Ignore && !k.IsComputed).Select(c => $"@{c.Name} = value.{c.PropertyName}"));
			DeleteParameter = string.Join(", ", Columns.Where(c => c.IsPK && !c.Ignore).Select(c => $"{c.Name} = value.{c.PropertyName}"));
			...
			PkParameter = string.Join(" , ", Columns.Where(c => c.IsPK).Select(c => $"@{c.Name} = {c.PropertyName}"));
		}
