...
namespace Greenshades.Online.TemplateGeneration.TemplateCodeGenerator {
	...
	class ConnectionStringProvider {
		...
		public EnvDTE.Project GetCurrentProject() {
			EnvDTE.DTE dte = new EnvDTE.DTEClass().DTE;

			if (dte == null) {
				throw new Exception("Unable to retrieve EnvDTE.DTE");
			}

			List<EnvDTE.Project> activeSolutionProjects = new List<EnvDTE.Project>();
			if (activeSolutionProjects == null) {
				throw new Exception("DTE.ActiveSolutionProjects returned null");
			}

			EnvDTE.Project dteProject = (EnvDTE.Project)activeSolutionProjects[0];
			if (dteProject == null) {
