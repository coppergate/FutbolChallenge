...
using System.IO;
using System.Linq;
using TemplateGeneratorCore.Repo.SchemaRead;
	...
	public class DacPacSchemaReader : SchemaReader {
		...
		override public Tables GetTables() {
			...
			try {
				...
				using (Stream dacPac = new MemoryStream((byte[])Resource.ResourceManager.GetObject(DacPacResourceName)))
				using (DacPackage package = DacPackage.Load(dacPac, DacSchemaModelStorageType.Memory))
				using (TSqlModel sqlModel = TSqlModel.LoadFromDacpac(dacPac, new ModelLoadOptions() { LoadAsScriptBackedModel = false, ModelStorageType = DacSchemaModelStorageType.Memory })) {
